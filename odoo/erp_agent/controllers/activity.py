...
class ActivityController(http.Controller):
    ...
    def activity(self, days=50, **kw):
        ...
        # parallel scan: errors per day across the same window
        err_msgs = Msg.search([
            ("role", "=", "error"),
            ("create_date", ">=", fields.Datetime.to_string(cutoff)),
        ], limit=2000)
        ...
        # 30d per-agent (independent of slider, so the row badge is stable)
        cutoff_30d = datetime.now() - timedelta(days=30)
        per_agent_30d: dict = {}
        agent30_msgs = Msg.search([
            ("role", "=", "assistant"),
            ("create_date", ">=", fields.Datetime.to_string(cutoff_30d)),
            ("agent_name", "!=", False),
        ], limit=5000)
        for am in agent30_msgs:
            per_agent_30d[am.agent_name] = per_agent_30d.get(am.agent_name, 0) + 1
